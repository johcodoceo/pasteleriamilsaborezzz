...
window.Validaciones = (function () {
    ...
    // Decisión del prototipo: el enunciado no especifica un máximo para mensajes.
    ...
    function validarProducto(datos, categorias) {
        ...
        const resultados = {
            ...
            personalizable: datos.personalizable === undefined || typeof datos.personalizable === "boolean"
                ? "" : "Indica si el producto permite un mensaje.",
        };
    ...
    // 6. PERSONALIZACIÓN: SE COMPARA EL MENSAJE CONSERVANDO SU ESCRITURA

    function normalizarMensajeTorta(valor) {
        return typeof valor === "string" ? valor.trim().normalize("NFC") : "";
    }

    function validarMensajeTorta(producto, valor) {
        if (typeof valor !== "string") {
            return "Escribe el mensaje como texto.";
        }

        if (/[\u0000-\u001f\u007f-\u009f\u2028\u2029]/.test(valor)) {
            return "Escribe el mensaje en una sola línea, sin saltos ni tabulaciones.";
        }

        const mensaje = normalizarMensajeTorta(valor);

        if (mensaje && !producto?.personalizable) {
            return "Este producto no admite mensajes personalizados.";
        }

        if (mensaje.length > MAX_MENSAJE_TORTA) {
            return `El mensaje puede tener hasta ${MAX_MENSAJE_TORTA} caracteres.`;
        }
